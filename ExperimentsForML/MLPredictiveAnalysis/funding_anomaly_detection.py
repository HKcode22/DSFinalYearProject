...
class FundingAnomalyDetection:
    ...
    def generate_anomaly_report(self, data, output_dir=None):
        ...
        Args:
            data (pd.DataFrame): The dataset to analyze for anomalies
            output_dir (str, optional): Directory to save the report. Defaults to self.output_dir.

        Returns:
            tuple: (anomalies DataFrame, report file path)
        """
        ...
        self.logger.info(f"Detected {total_anomalies} anomalies out of {total_companies} companies ({anomaly_rate:.2%})")
    ...
    def run_analysis(self):
        ...
        # Generate report
        anomalies, report_path = self.generate_anomaly_report(results)
...
def main():
    ...
    while True:
        try:
            ...
            anomalies, report_path = anomaly_detector.run_analysis()
