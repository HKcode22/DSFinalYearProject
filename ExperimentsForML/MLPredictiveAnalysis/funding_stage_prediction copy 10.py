...
class FeatureEngineering:
    ...
    def extract_features(self, df):
        ...
        if 'funding_amount_log' in data.columns and 'employee_efficiency' in data.columns:
            data['funding_x_efficiency'] = data['funding_amount_log'] * data['employee_efficiency']
        else:
            data['funding_x_efficiency'] = 0

        if 'months_since_first_funding' in data.columns and 'previous_rounds' in data.columns:
             data['age_x_rounds'] = data['months_since_first_funding'] * data['previous_rounds']
        else:
             data['age_x_rounds'] = 0
        # +++ End adding interaction features +++
        ...
        numeric_cols = [
            ...
            'funding_x_efficiency', # Include new features
            'age_x_rounds' # Include new features
        ]
    ...
    def prepare_model_data(self, data):
        ...
        feature_cols = [
            ...
            'month_sin', 'month_cos', # <<< ADD NEW FEATURES TO LIST
            'funding_amount_x_age', 'employees_x_rounds', # <<< ADD NEW FEATURES TO LIST
            'funding_x_efficiency', # <<< ADD NEW FEATURES TO LIST
            'age_x_rounds' # <<< ADD NEW FEATURES TO LIST
        ]
...
class EnhancedModelTrainer(ModelTrainer):
    def tune_random_forest(self, X, y):
        ...
        try: # Add try block
            ...
            grid_search = RandomizedSearchCV(
                ...
                # n_iter=4,  # <<< REDUCED FROM 8 to 4
                n_iter=16, # <<< Increased n_iter to 16
                cv=3,     # (Keep cv=3)
                verbose=1,
                random_state=42,
                n_jobs=-1,
    ...
    def tune_gradient_boosting(self, X, y, n_iter=16): # <<< Added n_iter parameter with default
        ...
        param_grid = {
            ...
            'learning_rate': uniform(0.01, 0.1),    # Focused range
            'max_depth': randint(3, 10),             # Keep reasonable depth
            'min_samples_split': randint(2, 20),
            'min_samples_leaf': randint(1, 20),
            'subsample': uniform(0.6, 0.4), # Range from 0.6 to 1.0
            'max_features': ['sqrt', 'log2', None] # Keep options
        }
        ...
        search = RandomizedSearchCV(
            estimator=gb,
            param_distributions=param_grid,
            # n_iter=n_iter,  # <<< Use passed n_iter value
            n_iter=16, # <<< Increased n_iter to 16
    ...
    def train_catboost(self, X, y):
        ...
        try:
            ...
            search = RandomizedSearchCV(
                cat_clf, param_grid, n_iter=16, cv=3, # <<< Increased n_iter to 16
    ...
    def tune_lightgbm(self, X, y, n_iter=10):
...
class EnhancedPipeline(FundingStagePredictionPipeline):
    ...
    def run(self):
        try:
            ...
            rf_model, rf_results = self._train_final_model(
                 default_rf, X.copy(), y.copy(), 'Random Forest' # Pass default model
            )


            # --- Use Default XGBoost --- #
            # logger.info("Tuning XGBoost (n_iter=4, cv=3)...") # <<< REMOVED TUNING
            # best_xgb = self.model_trainer.tune_xgboost(X, y)
            # if not best_xgb or isinstance(best_xgb, dict):
            #      logger.error("Hyperparameter tuning failed for XGBoost. Exiting.")
            #      return False
            # # Train final tuned XGB model
            ...
            try:
                 ...
                 # === Correctly place the training call inside the try block ===
                 xgb_model, xgb_results = self._train_final_model(
                      default_xgb, X.copy(), y.copy(), 'XGBoost' # Pass default model
                 )
            except ImportError:
            ...
            # logger.info("Tuning Gradient Boosting (n_iter=4, cv=3)...") # <<< REMOVED TUNING
            # # best_gb = self.model_trainer.tune_gradient_boosting(X, y, n_iter=4) # <<< Set n_iter=4
            # if not best_gb or isinstance(best_gb, dict): # Check if tuning failed
            #      logger.error("Hyperparameter tuning failed for Gradient Boosting. Exiting.") # Adjusted Message
            #      return False
            ...
            gb_model, gb_results = self._train_final_model(
                default_gb, X.copy(), y.copy(), 'Gradient Boosting' # Pass default model
            )

            # --- Tune LightGBM --- #
            logger.info("Tuning LightGBM (n_iter=10, cv=3)...")
            if LGBMClassifier:
                best_lgbm = self.model_trainer.tune_lightgbm(X, y, n_iter=10)
                if not best_lgbm:
                     logger.error("Hyperparameter tuning failed for LightGBM. Using defaults.")
                     best_lgbm = LGBMClassifier(random_state=42, n_jobs=-1) # Fallback to default

                lgbm_model, lgbm_results = self._train_final_model(
                    best_lgbm, X.copy(), y.copy(), 'LightGBM'
                )
            else:
                logger.warning("Skipping LightGBM tuning/training as it is not installed.")
                lgbm_model, lgbm_results = None, None
            # --- End Tune LightGBM --- #

            # --- Manual Stacking Ensemble using Calibrated Probabilities --- #
            logger.info("Training and evaluating Stacking Ensemble (Logistic Regression Meta-Model)...")
            stacking_ensemble_results = {} # Initialize results dict
            meta_model = None # Initialize model variable
            try:
                # Check which base models successfully trained and calibrated
                base_model_preds_status = {
                    'RF': rf_results, 'XGB': xgb_results, 'GB': gb_results, 'LGBM': lgbm_results
                }
                valid_base_models_for_stacking = []
                error_occurred = False
                for name, results in base_model_preds_status.items():
                     if results and results.get('calibrated_model'):
                         valid_base_models_for_stacking.append(name)
                     # Only raise error if a NON-LGBM model failed OR if LGBM failed *and* it was installed
                     elif name != 'LGBM' or LGBMClassifier:
                         logger.error(f"Base model {name} failed training/calibration. Cannot build ensemble.")
                         error_occurred = True

                if error_occurred or not valid_base_models_for_stacking:
                    raise ValueError("One or more required base models failed. Cannot build ensemble.")
                logger.info(f"Building ensemble using base models: {valid_base_models_for_stacking}")
                ...
                # Use a consistent scaler (find first valid one)
                scaler_ens = None
                for res_key in valid_base_models_for_stacking:
                     res = base_model_preds_status[res_key]
                     if res and res.get('scaler'):
                          scaler_ens = res['scaler']
                          break
                if not scaler_ens:
                     raise ValueError("Could not find a valid scaler from base models for ensemble.")

                X_train_ens_scaled = scaler_ens.transform(X_train_ens)
                X_test_ens_scaled = scaler_ens.transform(X_test_ens)

                # Get calibrated probabilities from VALID base models on TRAINING data
                train_probas = []
                base_feature_names = []
                n_classes = 0 # Determine number of classes from first valid model
                for model_key in valid_base_models_for_stacking:
                     results = base_model_preds_status[model_key]
                     model_proba = results['calibrated_model'].predict_proba(X_train_ens_scaled)
                     train_probas.append(model_proba)
                     if n_classes == 0: n_classes = model_proba.shape[1]
                     base_feature_names.extend([f'{model_key.lower()}_p{i}' for i in range(n_classes)])

                # Create meta-features for training the meta-model
                X_meta_train = np.concatenate(train_probas, axis=1)

                # --- Train Logistic Regression meta-model --- #
                # logger.info(f"Training Logistic Regression meta-model on {X_meta_train.shape[1]} probability features...")
                # meta_model = LogisticRegression(random_state=42, max_iter=1000) # Switched meta-model
                # meta_model.fit(X_meta_train, y_train_ens)
                # logger.info("Logistic Regression Meta-model trained.")
                # --- End LR Meta-Model --- #

                # --- Train RandomForest meta-model --- #
                logger.info(f"Training RandomForest meta-model on {X_meta_train.shape[1]} probability features...")
                meta_model = RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1) # Use RandomForest meta-model
                meta_model.fit(X_meta_train, y_train_ens)
                logger.info("RandomForest Meta-model trained.")
                # --- End RF Meta-Model --- #

                # Get calibrated probabilities from VALID base models on TEST data
                test_probas = []
                for model_key in valid_base_models_for_stacking:
                     results = base_model_preds_status[model_key]
                     test_probas.append(results['calibrated_model'].predict_proba(X_test_ens_scaled))

                # Create meta-features for testing
                X_meta_test = np.concatenate(test_probas, axis=1)

                # Evaluate the meta-model on test meta-features
                y_pred_stack = meta_model.predict(X_meta_test)
                y_proba_stack = meta_model.predict_proba(X_meta_test)

                # Calculate metrics for the stacking ensemble
                accuracy_stack = accuracy_score(y_test_ens, y_pred_stack)
                precision_stack = precision_score(y_test_ens, y_pred_stack, average='weighted', zero_division=0)
                recall_stack = recall_score(y_test_ens, y_pred_stack, average='weighted', zero_division=0)
                f1_stack = f1_score(y_test_ens, y_pred_stack, average='weighted', zero_division=0)
                rmse_stack = np.sqrt(mean_squared_error(y_test_ens, y_pred_stack))
                conf_matrix_stack = confusion_matrix(y_test_ens, y_pred_stack)
                report_labels_stack = np.unique(np.concatenate((y_test_ens, y_pred_stack)))
                class_report_stack = classification_report(y_test_ens, y_pred_stack, labels=report_labels_stack, output_dict=True, zero_division=0)

                stacking_ensemble_results = {
                    'model': meta_model,
                    'calibrated_model': meta_model,
                    'accuracy': accuracy_stack,
                    'precision': precision_stack,
                    'recall': recall_stack,
                    'f1_score': f1_stack,
                    'rmse': rmse_stack,
                    'confusion_matrix': conf_matrix_stack,
                    'classification_report': class_report_stack,
                    'cross_val_scores': np.array([np.nan]),
                    'X_test': X_meta_test,
                    'y_test': y_test_ens,
                    'y_pred': y_pred_stack,
                    'y_proba': y_proba_stack,
                    'feature_names': base_feature_names, # Use dynamically created names
                    'scaler': None
                }

                # Log performance
                logger.info(f"--- Stacking Ensemble (LR Meta-Model) Performance ---")
                logger.info(f"Accuracy: {accuracy_stack:.4f} ({accuracy_stack*100:.2f}%)")
                logger.info(f"Precision: {precision_stack:.4f} ({precision_stack*100:.2f}%)")
                logger.info(f"Recall: {recall_stack:.4f} ({recall_stack*100:.2f}%)")
                logger.info(f"F1 Score: {f1_stack:.4f} ({f1_stack*100:.2f}%)")
                logger.info(f"RMSE: {rmse_stack:.4f} (lower is better)")
                logger.info("-------------------------")
                print(f"\n--- Stacking Ensemble (LR Meta-Model) Performance ---")
                print(f"Accuracy: {accuracy_stack:.4f} ({accuracy_stack*100:.2f}%)")
                print(f"Precision: {precision_stack:.4f} ({precision_stack*100:.2f}%)")
                print(f"Recall: {recall_stack:.4f} ({recall_stack*100:.2f}%)")
                print(f"F1 Score: {f1_stack:.4f} ({f1_stack*100:.2f}%)")
                print(f"RMSE: {rmse_stack:.4f} (lower is better)")
                print("-------------------------")

            except Exception as stack_err:
                 logger.error(f"Error training or evaluating stacking ensemble: {stack_err}")
                 # Ensure results dict has Nones/NaNs if failed
                 stacking_ensemble_results = {
                    'model': None, 'calibrated_model': None,
                    ...
                    'cross_val_scores': np.array([np.nan]),
                    'X_test': None, 'y_test': None, 'y_pred': None, 'y_proba': None,
                    'feature_names': None, 'scaler': None
                 }
            # --- End Manual Stacking --- #

            # --- Fit Anomaly Detector --- #
            # Use scaled data from the *training* split of the best base model for fitting
            try:
            ...
            self._create_advanced_visualizations(
                ...
                xgb_results,
                gb_results,
                stacking_ensemble_results, # <<< Pass stacking results instead
            ...
            model_results_summary = {
                'Random Forest (Calibrated)': rf_results,
                'XGBoost (Calibrated)': xgb_results,
                'Gradient Boosting (Calibrated)': gb_results,
                # 'Optimized Ensemble (Calibrated)': opt_ensemble_results
                'Stacking Ensemble (Calibrated)': stacking_ensemble_results # <<< Use stacking results with new name
            }

            summary = self._save_summary(merged_data, X, model_results_summary)

            # Determine the best model to save based on summary results
            best_model_name = summary.get('best_model_by_accuracy') # Name will include (Calibrated) or (GB Meta) or (RF Meta)
            model_to_save = None # This should be the CALIBRATED model instance or the Meta Model
            scaler_to_save = None
            metadata_to_save = {}
            ...
            if best_model_name and best_model_name in model_results_summary:
                 ...
                 model_to_save = results_to_use.get('calibrated_model') # <<< SAVE CALIBRATED MODEL
                 ...
                 # Fallback to saving the Stacking ensemble if determination failed
                 logger.warning(f"Could not determine best model from summary. Saving Stacking Ensemble by default.")
                 best_model_name = 'Stacking Ensemble (RF Meta)' # <<< Adjusted default name to RF Meta
                 results_to_use = stacking_ensemble_results
                 model_to_save = results_to_use.get('model') # Save the meta-model
                 # scaler_to_save = results_to_use.get('scaler') # Scaler is None for meta-model results
                 # --- FIX: Get scaler from a base model when saving ensemble --- #
                 if gb_results and gb_results.get('scaler'):
                      scaler_to_save = gb_results['scaler']
                 elif xgb_results and xgb_results.get('scaler'):
                      scaler_to_save = xgb_results['scaler']
                 elif rf_results and rf_results.get('scaler'):
                      scaler_to_save = rf_results['scaler']
                 else:
                      scaler_to_save = None # Should not happen if base models trained
                      logger.error("Cannot find scaler from base models to save with ensemble!")
                 # --- END FIX --- #
                 metadata_to_save = summary['metrics'].get(best_model_name, {})
                 feature_names_to_save = self.feature_names # Need original features
    ...
    def _train_final_model(self, model, X, y, name):
        ...
        try:
             ...
             # Still return results dict, but with None for models
    ...
    def _save_summary(self, merged_data, X, model_results_summary): # model_results_summary includes LGBM and updated ensemble name
        ...
        summary = {
            ...
            'class_mapping': {int(k): v for k, v in self.final_class_mapping.items()},
            'metrics': {}
        }

        # --- Add Per-Stage Benchmark Statistics Calculation ---
        try:
            # Combine X features with the final y labels for grouping
            # Ensure indices align if they were reset during preprocessing
            if not isinstance(X, pd.DataFrame):
                X_df_bench = pd.DataFrame(X, columns=self.feature_names)
            else:
                X_df_bench = X.copy()

            # Use the final, remapped y series associated with X
            y_series_bench = y.reset_index(drop=True) # Use the final y passed to models
            X_df_bench['funding_stage_numeric'] = y_series_bench # Add the target column

            # Define key features for benchmarking
            benchmark_features = [
                'funding_amount', 'employees', 'months_since_first_funding',
                'previous_rounds', 'funding_velocity', 'employee_efficiency'
            ]
            # Filter to only features present in the data
            actual_benchmark_features = [f for f in benchmark_features if f in X_df_bench.columns]

            if actual_benchmark_features:
                # Calculate median, q1 (25th percentile), q3 (75th percentile) per stage
                benchmark_stats = X_df_bench.groupby('funding_stage_numeric')[actual_benchmark_features].agg(
                    ['median', lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)]
                )
                # Rename quantile columns for clarity
                benchmark_stats.columns = ['_'.join(col).strip('_') for col in benchmark_stats.columns.values]
                benchmark_stats = benchmark_stats.rename(columns={
                    '<lambda_0>': 'q1',
                    '<lambda_1>': 'q3'
                })
                # Convert to nested dictionary format suitable for JSON
                summary['benchmark_stats'] = benchmark_stats.unstack().unstack(0).to_dict()
                logger.info("Calculated and added benchmark statistics to summary.")
            else:
                summary['benchmark_stats'] = None
                logger.warning("No benchmark features found in data. Skipping benchmark stats calculation.")

        except Exception as bench_err:
            logger.error(f"Error calculating benchmark statistics: {bench_err}")
            summary['benchmark_stats'] = None
        # --- End Benchmark Statistics Calculation ---
        ...
        best_accuracy = -1.0
        valid_models_for_saving = {}

        for model_name, metrics in summary['metrics'].items():
            if metrics is not None and not np.isnan(metrics.get('accuracy', np.nan)):
                valid_models_for_saving[model_name] = metrics['accuracy']
                if metrics['accuracy'] > best_accuracy:
                    best_accuracy = metrics['accuracy']
                    best_overall_model_name = model_name

        logger.info(f"Determined best overall model by accuracy: {best_overall_model_name} (Accuracy: {best_accuracy:.4f})")

        # Save the extracted FI (if any) into the summary dictionary
        if importance_source_model and best_base_model_importance is not None:
            feature_importance_dict = {
                feature: float(imp) for feature, imp in zip(feature_names_imp, best_base_model_importance)
            }
            # <<< FIX: Assign correctly to summary dict >>>
            summary['feature_importance'] = {
                 'source_model': importance_source_model,
                 'importance': dict(sorted(feature_importance_dict.items(), key=lambda x: x[1], reverse=True))
            }
            logger.info(f"Added feature importance from {importance_source_model} to summary.") # Added log
        else:
             summary['feature_importance'] = None # Ensure key exists even if null
             logger.warning("Could not extract valid feature importance from any base model for summary.")

        # Determine components for the best model
        model_to_save = None
        scaler_to_save = None
        ...
        if best_overall_model_name and best_overall_model_name in model_results_summary:
            ...
            if results_to_use:
                ...
                # Logic for BASE models
                if 'Stacking Ensemble' not in best_overall_model_name: # Check if it's a base model
                    model_to_save = results_to_use.get('calibrated_model')
                    scaler_to_save = results_to_use.get('scaler')

                # Logic for ENSEMBLE model
                else: # It's the stacking ensemble
                    model_to_save = results_to_use.get('model') # Meta-model
                    # Find scaler from base models
                    scaler_found = False
                    for base_key in ['LightGBM', 'Gradient Boosting', 'XGBoost', 'Random Forest']:
                        if base_key in model_results_summary and model_results_summary[base_key] and model_results_summary[base_key].get('scaler'):
                            scaler_to_save = model_results_summary[base_key]['scaler']
                            logger.info(f"Using scaler from {base_key} for saving ensemble.")
                            scaler_found = True
                            break
                    if not scaler_found: logger.error("Scaler not found for ensemble.")

                # Final check on components
        ...
        # --- Actual Saving Call --- #
        # <<< Move the feature_names_to_save assignment here >>>
        feature_names_to_save = self.feature_names

        if best_overall_model_name and model_to_save and scaler_to_save:
            # Sanitize name for filename
            safe_model_name = best_overall_model_name.replace(" ", "_").replace("(", "").replace(")", "")
            # <<< Pass metadata_to_save which was determined earlier >>>
            saved_path = self.model_manager.save_model(
                model_name=safe_model_name,
                model=model_to_save,
                scaler=scaler_to_save,
                feature_names=feature_names_to_save,
                metadata=metadata_to_save # Use the metadata derived for the best model
            )
            if saved_path:
                 logger.info(f"Successfully saved best model ({best_overall_model_name}) and scaler to {saved_path}")
                summary['saved_model_path'] = saved_path # <<< Add saved path to summary >>>
                summary['saved_model_name'] = best_overall_model_name # <<< Add model name to summary >>>
            else:
                 logger.error(f"Failed to save the best model ({best_overall_model_name}).")
        else:
            # <<< Modify this log message >>>
            logger.error(f"Could not save best model ('{best_overall_model_name}') - essential components (model/scaler) missing or name invalid.")
        # --- End Saving Call --- #
        ...
        try:
             with open(summary_path, 'w') as f:
                  # Use NumpyEncoder to handle potential numpy types in metrics/reports
        ...
        # <<< Adjusted width for potentially longer model names >>>
        print(f"{'Model':<35} {'Accuracy':<12} {'Precision':<12} {'Recall':<12} {'F1 Score':<12} {'RMSE':<12}")
        print("-" * 100) # Adjusted separator length

        # <<< Use items() to iterate through metrics dictionary >>>
        for model_name, metrics in summary.get('metrics', {}).items():
            if metrics is None:
                 acc, prec, rec, f1, rmse_val = ('N/A',) * 5
            else:
                 # Format metrics, handling potential NaN
                 acc = f"{metrics.get('accuracy', np.nan) * 100:.2f} %" if not np.isnan(metrics.get('accuracy', np.nan)) else 'N/A'
                 prec = f"{metrics.get('precision_weighted', np.nan) * 100:.2f} %" if not np.isnan(metrics.get('precision_weighted', np.nan)) else 'N/A'
                 rec = f"{metrics.get('recall_weighted', np.nan) * 100:.2f} %" if not np.isnan(metrics.get('recall_weighted', np.nan)) else 'N/A'
                 f1 = f"{metrics.get('f1_weighted', np.nan) * 100:.2f} %" if not np.isnan(metrics.get('f1_weighted', np.nan)) else 'N/A'
                 rmse_val = metrics.get('rmse')
            rmse = f"{rmse_val:.4f}" if rmse_val is not None and not np.isnan(rmse_val) else 'N/A'

            # <<< Print formatted row >>>
            print(f"{model_name:<35} {acc:<12} {prec:<12} {rec:<12} {f1:<12} {rmse:<12}")
...
class DashboardInterface:
    """Provides methods to fetch data needed for the prediction dashboard."""
    def __init__(self, output_dir="./MainOutput"):
        self.output_dir = output_dir
        self.models_dir = os.path.join(output_dir, "models")
        self.viz_dir = os.path.join(output_dir, "visualizations")
        # Correct path for the interactive plot
        self.dashboard_viz_dir = os.path.join(output_dir, "prototype_dashboard") 

        self.model_manager = ModelManager(self.models_dir)
        self.summary_file_path = None
        self.latest_summary = None
        self.reverse_class_mapping = {} # Will be loaded from summary

        # Find and load the latest summary file
        self._load_latest_summary()

        # Initialize paths for plots (will find latest in methods)
        self.timeseries_plot_path = None
        self.calibration_plot_path = None
    ...
    def _find_latest_file(self, directory, pattern):
        ...
        try:
            ...
            logger.error(f"Error finding latest file in {directory} with pattern {pattern}: {e}")
            return None

    def _load_latest_summary(self):
        """Loads the latest summary JSON file."""
        self.summary_file_path = self._find_latest_file(self.output_dir, "summary_*.json")
        if self.summary_file_path and os.path.exists(self.summary_file_path):
            try:
                with open(self.summary_file_path, 'r') as f:
                    self.latest_summary = json.load(f)
                # Load the reverse mapping from the summary
                if 'class_mapping' in self.latest_summary and isinstance(self.latest_summary['class_mapping'], dict): # Check type
                     # Original mapping is label: index. We need index: label
                     original_mapping = self.latest_summary['class_mapping']
                     # Ensure keys from JSON (strings) are converted back to int for mapping
                     self.reverse_class_mapping = {int(v): str(k) for k, v in original_mapping.items()}
                     logger.info(f"Loaded reverse class mapping from summary: {self.reverse_class_mapping}")
                else:
                     logger.warning("Class mapping not found or invalid in the latest summary file.")
                logger.info(f"Loaded latest summary from: {self.summary_file_path}")
            except Exception as e:
                logger.error(f"Error loading summary file {self.summary_file_path}: {e}")
                self.latest_summary = None
        else:
            logger.warning("Could not find or load latest summary file.")
            self.latest_summary = None

    # --- Methods for Dashboard Sections will go here ---
    def get_prediction(self, input_features):
        """Loads the best model and returns prediction for input features.

        Args:
            input_features (dict): Dictionary with feature names and values.
                                   Must include all features required by the model.

        Returns:
            dict: Prediction results including predicted stage, confidence,
                  probabilities (if available), and validation info.
        """
        try:
            # Determine the best model name from the summary
            best_model_name = None
            safe_model_name_for_load = None # Initialize
            if self.latest_summary and 'saved_model_name' in self.latest_summary:
                best_model_name = self.latest_summary['saved_model_name']
                safe_model_name_for_load = best_model_name.replace(" ", "_").replace("(", "").replace(")", "")
            else:
                logger.warning("Saved model name not found in summary. Attempting common names...")
                possible_names = ["Optimized_Ensemble", "Stacking_Ensemble_RF_Meta", "XGBoost", "Gradient_Boosting", "Random_Forest"]
                loaded_ok = False
                for name_to_try in possible_names:
                    # Check if model file exists before attempting to load
                    model_files_pkl = glob.glob(os.path.join(self.models_dir, f"{name_to_try}*.pkl"))
                    model_files_joblib = glob.glob(os.path.join(self.models_dir, f"{name_to_try}*.joblib"))
                    if model_files_pkl or model_files_joblib:
                         if self.model_manager.load_model_joblib(model_name=name_to_try, version='latest'):
                             best_model_name = name_to_try
                             safe_model_name_for_load = best_model_name # Use the name that loaded
                             loaded_ok = True
                             logger.info(f"Falling back to loading model: {best_model_name}")
                             break # Exit loop once a model is loaded
                         else:
                              logger.warning(f"Found files for {name_to_try} but failed to load.")
                    else:
                         logger.debug(f"No model files found for fallback option: {name_to_try}")
                if not loaded_ok:
                    logger.error("Could not load any fallback trained model for prediction.")
                    return {'error': 'No suitable model found', 'is_valid': False}
            
            # Explicitly load the determined model if not loaded during fallback
            if not self.model_manager.model and safe_model_name_for_load:
                if not self.model_manager.load_model_joblib(model_name=safe_model_name_for_load, version='latest'):
                    logger.error(f"Failed to load the specified best model '{best_model_name}' ({safe_model_name_for_load}).")
                    return {'error': f"Failed to load model '{best_model_name}'", 'is_valid': False}
            elif not self.model_manager.model:
                 # This should only happen if summary loading failed AND fallback failed
                 logger.error("No model is loaded, and could not determine which model to load.")
                 return {'error': 'Model loading failed', 'is_valid': False}

            # Ensure scaler and feature names are loaded by load_model_joblib
            if self.model_manager.scaler is None or not self.model_manager.feature_names:
                logger.error("Scaler or required feature names not loaded with the model.")
                return {'error': 'Model loaded without scaler/feature names', 'is_valid': False}
            
            required_features = self.model_manager.feature_names
            
            # === Strict Input Validation ===
            if not isinstance(input_features, dict):
                 return {'error': 'Invalid input_features format. Provide dict.', 'is_valid': False}
            ...
            for feature in required_features:
                if feature not in input_features or input_features[feature] is None or input_features[feature] == '': # Check for missing, None, or empty string
                    missing_features_list.append(feature)
                else:
                    try:
                        formatted_features[feature] = float(input_features[feature])
                    except (ValueError, TypeError):
                        type_error_features[feature] = input_features[feature]
            
            # Return error if any features are missing or have type errors
            error_messages = []
            if missing_features_list:
                error_messages.append(f'Missing required features: {", ".join(missing_features_list)}')
            if type_error_features:
                 error_messages.append(f'Invalid type for features: {json.dumps(type_error_features)}')
            
            if error_messages:
                 return {'error': "; ".join(error_messages), 'is_valid': False}
            # === End Strict Input Validation ===

            # Make prediction using the loaded model and manager's predict method
            # This method now handles scaling internally if a scaler is present
            prediction_result = self.model_manager.predict(formatted_features) # Pass the validated dictionary

            # Map back to original funding stage using the reverse mapping
            if 'prediction' in prediction_result and self.reverse_class_mapping:
                 predicted_class_index = prediction_result['prediction']
                 predicted_stage_label = self.reverse_class_mapping.get(
                     int(predicted_class_index),
                     f"Unknown Stage (Index {predicted_class_index})")
                 prediction_result['predicted_stage_label'] = predicted_stage_label
            elif 'error' not in prediction_result:
                 logger.warning("Could not map prediction index back to stage label (mapping missing or prediction failed).")
                 if 'prediction' in prediction_result:
                     prediction_result['predicted_stage_label'] = f"Unknown Stage (Index {prediction_result['prediction']})"

            # Add probability distribution for top N classes
            if hasattr(self.model_manager.model, 'predict_proba') and 'error' not in prediction_result:
                 try:
                     features_array = np.array([formatted_features[f] for f in required_features]).reshape(1, -1)
                     features_scaled = self.model_manager.scaler.transform(features_array)
                     probabilities = self.model_manager.model.predict_proba(features_scaled)[0]
                     classes = self.model_manager.model.classes_ if hasattr(self.model_manager.model, 'classes_') else list(range(len(probabilities)))
                     
                     prob_dict = {
                         self.reverse_class_mapping.get(int(cls_idx), f"Unknown_{cls_idx}"): float(prob)
                         for cls_idx, prob in zip(classes, probabilities)
                     }
                     sorted_probs = dict(sorted(prob_dict.items(), key=lambda item: item[1], reverse=True)[:5])
                     prediction_result['probability_distribution'] = sorted_probs
                 except Exception as proba_err:
                     logger.warning(f"Could not calculate probability distribution: {proba_err}")
                     prediction_result['probability_distribution'] = None
    ...
    def get_general_feature_importance(self):
        """Retrieves the general feature importance from the latest summary.

        Returns:
            dict: Sorted dictionary of feature: importance score, or None if not found.
        """
        if self.latest_summary and 'feature_importance' in self.latest_summary and self.latest_summary['feature_importance']:
            importance_data = self.latest_summary['feature_importance']
            return {
                'source_model': importance_data.get('source_model', 'Unknown'),
                'importance': importance_data.get('importance', {})
            }
        else:
            logger.warning("Feature importance data not found in the latest summary.")
            return None

    def get_benchmark_data(self, predicted_stage_numeric, input_features):
        """Compares input features to typical values for the predicted stage.

        Args:
            predicted_stage_numeric (int): The numeric index of the predicted stage.
            input_features (dict): The user's input features (already validated and numeric).

        Returns:
            dict: Comparison results for key features, or None.
        """
        if not self.latest_summary or 'benchmark_stats' not in self.latest_summary or not self.latest_summary['benchmark_stats']:
             logger.warning("Benchmark statistics not found in summary. Cannot perform comparison.")
             return None
        
        per_stage_stats = self.latest_summary['benchmark_stats']
        predicted_stage_str = str(predicted_stage_numeric) # Keys in JSON dict are strings

        if predicted_stage_str not in per_stage_stats:
             logger.warning(f"Benchmark statistics not found for predicted stage {predicted_stage_numeric} in summary.")
             return None

        stage_stats = per_stage_stats[predicted_stage_str]
        benchmark_results = {}
        
        # Define key features for benchmarking (should match those calculated in _save_summary)
        benchmark_features = [
            'funding_amount', 'employees', 'months_since_first_funding',
            'previous_rounds', 'funding_velocity', 'employee_efficiency'
        ]
        ...
        for feature in benchmark_features:
            if feature in input_features and feature in stage_stats:
                user_value = input_features[feature]
                median_value = stage_stats[feature].get('median')
                q1_value = stage_stats[feature].get('q1')
                q3_value = stage_stats[feature].get('q3')

                comparison_text = f"User: {user_value:,.2f}. " # Format user value
                if median_value is not None:
                    comparison_text += f"Typical Median: {median_value:,.2f}. "
                if q1_value is not None and q3_value is not None:
                    comparison_text += f"Typical Range (IQR): {q1_value:,.2f} - {q3_value:,.2f}."
                    # Add qualitative comparison
                    if user_value < q1_value:
                        comparison_text += " (Below typical range)"
                    elif user_value > q3_value:
                        comparison_text += " (Above typical range)"
                    else:
                        comparison_text += " (Within typical range)"
                
                benchmark_results[feature] = comparison_text.strip()
            elif feature in input_features:
                 # Feature provided by user, but no benchmark stats available
                 benchmark_results[feature] = f"User: {input_features[feature]:,.2f}. (No benchmark data available for this stage)"

        return benchmark_results if benchmark_results else None

    def get_timeseries_plot_path(self):
        """Returns the path to the latest interactive timeseries plot HTML file."""
        # Search in the prototype_dashboard directory for HTML files matching the pattern
        plot_path = self._find_latest_file(self.dashboard_viz_dir, "bay_area_funding_trend_interactive_*.html")
        if plot_path:
             logger.info(f"Found latest timeseries plot: {plot_path}")
        else:
             logger.warning(f"No timeseries plot found in {self.dashboard_viz_dir}")
        return plot_path


    def get_calibration_plot_path(self):
        """Returns the path to the latest calibration plot PNG file."""
        # Search in the visualizations directory for calibration plot PNGs
        plot_path = self._find_latest_file(self.viz_dir, "calibration_plot_*.png") # Check for _* to match timestamp
        if not plot_path:
             # Fallback to older naming convention if no timestamped files found
             plot_path = self._find_latest_file(self.viz_dir, "calibration_plot.png")

        if plot_path:
             logger.info(f"Found latest calibration plot: {plot_path}")
        else:
             logger.warning(f"No calibration plot found in {self.viz_dir}")
        return plot_path
...
if __name__ == "__main__":
    ...
    Includes argument parsing for running the pipeline and a basic test of the DashboardInterface.
    """
    import argparse
    import schedule
    import time
    ...
    # --- Removed direct test calls from main ---
    # The test logic for DashboardInterface is now placed within a dedicated
    # test function that can be called if needed.
