...
class EnhancedModelTrainer(ModelTrainer):
    ...
    def tune_gradient_boosting(self, X, y, n_iter=16): # <<< Added n_iter parameter with default
        ...
        search = RandomizedSearchCV(
            ...
            n_iter=4,  # <<< REDUCED FROM 8/16 to 4
    ...
    def train_catboost(self, X, y):
        ...
        try:
            ...
            search = RandomizedSearchCV(
                cat_clf, param_grid, n_iter=4, cv=3, # <<< REDUCED FROM 8 to 4
...
class ModelManager:
    ...
    def save_model(
        ...
        Args:
            model_name: Namee to save the model under
...
class EnhancedPipeline(FundingStagePredictionPipeline):
    ...
    def run(self):
        try:
            ...
            model_results_summary_for_saving = {
                ...
                'LightGBM': lgbm_results, # LightGBM name doesn't include (Calibrated) in some places
    ...
    def _save_summary(self, merged_data, X, model_results_summary, processed_data): # <<< Added processed_data
        ...
        summary = {
            ...
            # Save the correct mapping: final numeric index -> string label
            'class_mapping': {int(k): str(v) for k, v in getattr(self, 'final_index_to_string_label_map', {}).items()},
            'metrics': {}
        }
        ...
        try:
             ...
             if 'funding_stage_numeric' in processed_data.columns and \
                'funding_amount' in processed_data.columns and \
                'employees' in processed_data.columns:

                 # Group by the final numeric stage
                 grouped = processed_data.groupby('funding_stage_numeric')

                 for stage_num, group_data in grouped:
                     # Ensure stage_num is a standard Python int for JSON compatibility
                     stage_key = int(stage_num)
                     benchmarks[stage_key] = {
                         'funding_amount_median': float(group_data['funding_amount'].median()),
                         'funding_amount_q1': float(group_data['funding_amount'].quantile(0.25)),
                         'funding_amount_q3': float(group_data['funding_amount'].quantile(0.75)),
                         'employees_median': float(group_data['employees'].median()),
                         'employees_q1': float(group_data['employees'].quantile(0.25)),
                         'employees_q3': float(group_data['employees'].quantile(0.75)),
                         'count': int(len(group_data)) # Add count for context
                     }
                 summary['benchmarks'] = benchmarks
                 logger.info("Successfully calculated and added benchmark statistics.")
             else:
        ...
        if best_overall_model_name and best_overall_model_name in model_results_summary:
            ...
            if results_to_use:
                ...
                if best_overall_model_name != 'Stacking Ensemble (LR Meta)':
                ...
                # Logic for ENSEMBLE model
                    ...
                    scaler_found = False
                    # <<< Use the correct keys with (Calibrated) where applicable >>>
                    for base_key_lookup in ['LightGBM', 'Gradient Boosting (Calibrated)', 'XGBoost (Calibrated)', 'Random Forest (Calibrated)']:
                        # Check if the key exists and the results are valid
                        if base_key_lookup in model_results_summary and model_results_summary[base_key_lookup] and model_results_summary[base_key_lookup].get('scaler'):
                            scaler_to_save = model_results_summary[base_key_lookup]['scaler']
                            logger.info(f"Using scaler from {base_key_lookup} for saving ensemble.")
                            scaler_found = True
                            break
