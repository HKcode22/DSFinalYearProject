    ...
    def tune_random_forest(self, X, y):
        ...
        try: # Add try block
            ...
            grid_search = RandomizedSearchCV(
                ...
                n_iter=1,  # <<< REDUCED FROM 8 back to 4
                cv=3,     # (Keep cv=3)
                verbose=1,
                random_state=42,
                n_jobs=-1,
    ...
    def run(self):
        try:
            ...
            try:
                 ...
                 xgb_model, xgb_results = None, None
            # --- Tune XGBoost --- #
            logger.info("Tuning XGBoost (n_iter=4, cv=3)...") # Using n_iter=4 as previously set
            ...
            # try:
                # ... (Keep Stacking Ensemble logic commented out)
...
# def apply_post_prediction_rules(prediction_result, input_features_dict):
#     """Applies heuristic rules to adjust model predictions based on input features.

#     Args:
#         prediction_result (dict): The dictionary returned by the model's prediction 
#                                    (must contain 'predicted_label', 'confidence').
#         input_features_dict (dict): The original dictionary of input features.

#     Returns:
#         dict: The potentially modified prediction_result dictionary with adjusted 
#               'predicted_label' and 'confidence' if a rule triggered.
#     """
#     adjusted_result = prediction_result.copy() # Work on a copy
#     predicted_label = adjusted_result.get("predicted_label")
#     # Use the final confidence (after potential anomaly adjustment) as the base for rule adjustments
#     current_confidence = adjusted_result.get("confidence", 0.0) 

#     # --- Extract common features needed for rules --- #
#     # Use .get() with defaults for robustness against missing inputs
#     months = pd.to_numeric(input_features_dict.get('months_since_first_funding'), errors='coerce')
#     rounds = pd.to_numeric(input_features_dict.get('previous_rounds'), errors='coerce')
#     funding = pd.to_numeric(input_features_dict.get('funding_amount'), errors='coerce')
#     employees = pd.to_numeric(input_features_dict.get('employees'), errors='coerce')

#     original_label = predicted_label # Store for logging

#     # --- Rule 1: Check for illogical "post-ipo" predictions for early-stage inputs --- #
#     if predicted_label == "post-ipo":
#         # Define 'early stage' criteria
#         is_very_young = months is not None and pd.notna(months) and months <= 24 # Changed < to <=
#         is_extremely_young = months is not None and pd.notna(months) and months < 6 # Added for Pre-Seed check
#         has_few_rounds = rounds is not None and pd.notna(rounds) and rounds <= 1
#         has_low_funding = funding is not None and pd.notna(funding) and funding < 20_000_000
#         has_few_employees = employees is not None and pd.notna(employees) and employees < 50

#         # Count signals
#         early_stage_signals = sum([is_very_young, has_few_rounds, has_low_funding, has_few_employees])

#         # Determine override target and apply if enough signals
#         if early_stage_signals >= 2:
#             if is_extremely_young: # Prioritize Pre-Seed for very new companies
#                 new_predicted_label = "Pre-Seed"
#             else: # Default override to Seed
#                 new_predicted_label = "Seed"

#             # Apply override
#             new_confidence = min(current_confidence * 0.5, 0.10) # Cap at 10%
#             adjusted_result["predicted_label"] = new_predicted_label
#             adjusted_result["confidence"] = new_confidence
#             logger.info(f"Post-Prediction Rule 1 Triggered: Overrode '{original_label}' to '{new_predicted_label}' due to {early_stage_signals} early-stage signals. Confidence set to {new_confidence:.3f}")

#     # --- Rule 2: Check for illogical LOW predictions for high-funding/late-stage inputs --- #
#     # Check if the FIRST rule already triggered; if so, skip this one
#     if adjusted_result.get("predicted_label") == original_label: 
#         is_very_high_funding = funding is not None and pd.notna(funding) and funding > 100_000_000 # Over $100M
#         is_quite_old = months is not None and pd.notna(months) and months > 48 # Over 4 years
#         has_many_rounds = rounds is not None and pd.notna(rounds) and rounds >= 3 # 3 or more rounds

#         # Define stages considered "too low" for such companies
#         too_low_stages = ["Pre-Seed", "Seed", "Series A", "Series B", "Grant", "venture - series unknown"] 

#         # Trigger if predicted stage is too low AND multiple late-stage signals are present
#         late_stage_signals = sum([is_very_high_funding, is_quite_old, has_many_rounds])
#         if predicted_label in too_low_stages and late_stage_signals >= 2: # If stage is low AND at least 2 signals match
#             # Override prediction to something more plausible (e.g., Series C or D? Let's try Series D)
#             new_predicted_label = "Series D" 
#             # Reduce confidence, but maybe less drastically than Rule 1
#             new_confidence = min(current_confidence * 0.7, 0.25) # Cap at 25%
#             adjusted_result["predicted_label"] = new_predicted_label
#             adjusted_result["confidence"] = new_confidence
#             logger.info(f"Post-Prediction Rule 2 Triggered: Overrode '{original_label}' to '{new_predicted_label}' due to late-stage signals. Confidence set to {new_confidence:.3f}")

#     # --- Rule 3: Correct 'post-ipo' predictions for likely mid/late venture stage --- #
#     # Check if Rule 1 or 2 already triggered
#     if adjusted_result.get("predicted_label") == original_label and predicted_label == "post-ipo":
#          # Define criteria that suggest mid/late venture, not IPO
#          is_not_extremely_young = months is not None and pd.notna(months) and months > 24
#          has_multiple_rounds = rounds is not None and pd.notna(rounds) and rounds >= 2
#          is_not_mega_funded = funding is not None and pd.notna(funding) and funding < 150_000_000 # Less than $150M

#          # Trigger if multiple signals match
#          mid_late_signals = sum([is_not_extremely_young, has_multiple_rounds, is_not_mega_funded])
#          if mid_late_signals >= 2: # Require at least 2 signals
#              new_predicted_label = "Series C" # Override to Series C as a guess
#              new_confidence = min(current_confidence * 0.6, 0.20) # Cap at 20%
#              adjusted_result["predicted_label"] = new_predicted_label
#              adjusted_result["confidence"] = new_confidence
#              logger.info(f"Post-Prediction Rule 3 Triggered: Overrode '{original_label}' to '{new_predicted_label}' due to mid/late-stage signals. Confidence set to {new_confidence:.3f}")
