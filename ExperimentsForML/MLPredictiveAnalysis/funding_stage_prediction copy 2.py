...
from prophet.plot import plot_plotly # Using plotly for potentially interactive plots if needed later
...
class EnhancedPipeline(FundingStagePredictionPipeline):
    ...
    def _create_advanced_visualizations(
        ...
        try:
            ...
            logger.info("Step 4.5: Performing Time Series Forecasting...")
            try:
                # Use processed_data_viz as it's the data before splitting/scaling
                prophet_data = self.time_series_forecaster.prepare_prophet_data(processed_data_viz)
                if prophet_data is not None and not prophet_data.empty:
                    # +++ Reduce forecast period to 12 months +++
                    ...
                    if prophet_model and prophet_forecast is not None:
                        ...
                        logger.warning("Prophet model training/prediction failed. Skipping forecast plot.")
                else:
                    logger.warning("Data preparation for Prophet failed or yielded no data. Skipping forecast.")
            except ImportError:
                logger.warning("Prophet library not found. Skipping time series forecasting. Please install it (`pip install prophet`).")
            except Exception as ts_err:
                 logger.error(f"Error during time series forecasting step: {ts_err}")
                 logger.error(traceback.format_exc())

            # --- Model Diagnostic Plots --- #
            logger.info("Step 5: Creating advanced visualizations...") # Renumber step
...
# +++ Add TimeSeriesForecaster Class +++
class TimeSeriesForecaster:
    ...
    def prepare_prophet_data(self, data):
        ...
        # Set date as index and resample monthly, calculating median stage and regressors
        monthly_data = data.set_index('funding_date')
        # Using median as it's more robust to outliers than mean for stages
        monthly_agg = monthly_data['funding_stage_numeric'].resample('MS').median() # 'MS' for Month Start frequency

        # Check if the result is a DataFrame or Series and rename accordingly
        if isinstance(monthly_agg, pd.DataFrame):
            # Rename the column resulting from 'size' aggregation
            if 'funding_stage_numeric' in monthly_agg.columns:
                monthly_agg = monthly_agg.rename(columns={'funding_stage_numeric': 'deal_count'})
        # If it's a Series (e.g., only one aggregation succeeded), handle differently
        # This part might need more logic depending on which aggregation produced the Series
        elif isinstance(monthly_agg, pd.Series):
             # If the series name is funding_stage_numeric (from size), rename it
             if monthly_agg.name == 'funding_stage_numeric':
                  monthly_agg.name = 'deal_count'
             # Convert Series to DataFrame to proceed consistently
             monthly_agg = monthly_agg.to_frame()

        # Reset index and rename columns for Prophet
        prophet_df = monthly_agg.reset_index()
        # Rename target and potential regressors based on actual columns from agg
        prophet_df = prophet_df.rename(columns={
            'funding_date': 'ds',
            'funding_amount_log': 'median_funding_log'
            # 'deal_count' is already named correctly from the rename above
        })

        # -- Crucial Fix: Ensure 'y' exists after aggregation --
        # Re-calculate median stage separately if 'size' overwrote it in the dict key
        if 'y' not in prophet_df.columns:
             median_stage = monthly_data['funding_stage_numeric'].resample('MS').median()
             prophet_df = prophet_df.merge(median_stage.rename('y'), left_on='ds', right_index=True)

        # Select and reorder columns for clarity
        prophet_df = prophet_df[['ds', 'y', 'median_funding_log', 'deal_count']]
 
        # Drop months with no data (NaN median)
        prophet_df = prophet_df.dropna()

        if len(prophet_df) < 12: # Need at least ~1 year of data for meaningful forecast
             logger.warning(f"Insufficient historical data ({len(prophet_df)} months) for reliable Prophet forecast.")
             # return None # Decide if we should stop or proceed with warning

        logger.info(f"Prepared {len(prophet_df)} monthly data points with regressors for Prophet.")
        return prophet_df

    def train_predict(self, prophet_df, periods=24, freq='MS'):
        ...
        logger.info(f"Training Prophet model to forecast {periods} periods ({freq})...")
        try:
