...
try:
    ...
    Prophet = None # Set to None if import fails
...
class TimeSeriesForecaster:
    ...
    def prepare_prophet_data(self, data):
        ...
        # +++ Log Raw Monthly Counts Before Aggregation (Filtered) +++
        raw_quarterly_counts = monthly_data.resample('QS').size() # Resample by Quarter Start
        logger.info(f"Raw quarterly deal counts (from {monthly_data.index.min().date()}, before aggregation/filtering):\n{raw_quarterly_counts.to_string()}")
        # +++ End Logging +++ #

        # --- Implement Rolling Window Aggregation (2-Quarter window) --- #
        window_size = 2 # Quarters
        ...
        # For deal count, get the size per quarter separately
        quarterly_counts = monthly_data.resample('QS').size()
        # Apply rolling sum to the counts
    ...
    def train_predict(self, prophet_df, periods=6, freq='MS'):
        ...
        if len(prophet_df) < min_data_points:
            ...
            logger.warning(f"Insufficient historical data ({len(prophet_df)} months) for reliable Prophet forecast. Minimum required: {min_data_points}. Skipping forecast.")
            return None, None
    ...
    def plot_dashboard_prototype(self, forecast, history_df):
        """Plot historical actuals and forecast for the dashboard prototype."""
        ...
        try:
            ...
            ax.plot(history_df['ds'], history_df['y'], 'ko-', label='Historical Median Stage', markersize=4) # Black dots and line

            # Plot forecast
            ax.plot(forecast['ds'].iloc[forecast_start_index:], forecast['yhat'].iloc[forecast_start_index:], ls='-', marker='o', markersize=4, c='blue', label='Forecast Median Stage') # Add markers
            ...
            # Formatting
            ax.set_title('Bay Area Startups: Historical & Forecasted Median Funding Stage (6 Months)', fontsize=14)
            ax.set_xlabel('Date')
            ...
            # Save the plot
            plot_path = os.path.join(prototype_output_dir, f"bay_area_funding_trend_{self.timestamp}.png")
            plt.tight_layout()
            plt.savefig(plot_path)
