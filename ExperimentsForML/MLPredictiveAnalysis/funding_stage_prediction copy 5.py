...
class TimeSeriesForecaster:
    ...
    def prepare_prophet_data(self, data):
        ...
        quarterly_agg['y'] = quarterly_resampled['funding_stage_numeric'].rolling(window=window_size, min_periods=1).median()
    ...
    def train_predict(self, prophet_df, periods=6, freq='MS'):
        ...
        logger.info(f"Training Prophet model to forecast {periods} quarters ({freq})...")
        try:
    ...
    def plot_dashboard_prototype(self, forecast, history_df):
        """Plot historical actuals and forecast (if provided) for the dashboard prototype."""
        # if forecast is None or forecast.empty: # Removed forecast check - optional now
        #     logger.error("Cannot plot dashboard prototype: Forecast data missing.")
        #     return
        if history_df is None or history_df.empty:
             logger.error("Cannot plot dashboard prototype: Historical data missing for comparison.")
             return

        logger.info("Generating dashboard prototype plot...")
        ...
        try:
            ...
            ax.plot(history_df['ds'], history_df['y'], 'ko-', label='Historical Rolling Median Stage (4-Qtr)', markersize=4) # Updated label

            # plot_title = 'Bay Area Startups: Historical Rolling Median Funding Stage (4-Quarter)' # Title set later

            # --- Plot forecast only if provided --- #
            if forecast is not None and not forecast.empty:
                # Plot historical fit (in-sample yhat)
                # Merge forecast yhat with history dates for easy plotting
                plot_df = history_df.merge(forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']], on='ds', how='left')
                ax.plot(plot_df['ds'], plot_df['yhat'], ls='--', c='red', label='Model Fit (Historical)') # Add model fit line

                # Find the date where forecast starts
                last_hist_date = history_df['ds'].max()
                forecast_start_index = forecast[forecast['ds'] > last_hist_date].index.min()

                # Plot forecast (out-of-sample yhat)
                ax.plot(forecast['ds'].iloc[forecast_start_index:], forecast['yhat'].iloc[forecast_start_index:], ls='-', marker='o', markersize=4, c='blue', label='Forecast Median Stage (2 Qtrs)') # Updated label

                # Plot confidence interval (only for forecast period)
                ax.fill_between(forecast['ds'].iloc[forecast_start_index:],
                                forecast['yhat_lower'].iloc[forecast_start_index:],
                                forecast['yhat_upper'].iloc[forecast_start_index:],
                                color='blue', alpha=0.2, label='95% Confidence Interval')
                plot_title = 'Bay Area Startups: Historical Trend, Model Fit & Forecast (4-Qtr Rolling)' # Update title if forecast shown
            else:
                 plot_title = 'Bay Area Startups: Historical Rolling Median Funding Stage (4-Quarter)' # Title if no forecast

            # --- End Forecast Plotting --- #

            # Formatting
            ax.set_title(plot_title, fontsize=14) # Use dynamic title
            ax.set_xlabel('Date (Quarter Start)') # Updated label
            ax.set_ylabel('Median Funding Stage (Numeric)')
            ...
            # Save the plot
            plot_path = os.path.join(prototype_output_dir, f"bay_area_funding_trend_hist_{self.timestamp}.png") # Updated filename
            plt.tight_layout()
            plt.savefig(plot_path)
            logger.info(f"Dashboard prototype plot saved to {plot_path}")
            plt.close(fig)

        except Exception as e:
            logger.error(f"Error generating dashboard prototype plot: {e}")
            logger.error(traceback.format_exc())
            plt.close() # Ensure plot is closed even if error occurs
