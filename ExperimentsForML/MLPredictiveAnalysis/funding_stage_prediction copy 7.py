...
class EnhancedModelTrainer(ModelTrainer):
    ...
    def train_catboost(self, X, y):
        ...
        try:
            ...
            search = RandomizedSearchCV(
                cat_clf, param_grid, n_iter=8, cv=3, # <<< REDUCED FROM 15 to 8
...
class EnhancedPipeline(FundingStagePredictionPipeline):
    ...
    def run(self):
        try:
            ...
            # --- Focus tuning on GB and XGB, use default for RF --- #
            logger.info("Skipping Random Forest tuning, using defaults.")
            # Train RF with defaults via _train_final_model (which now includes calibration)
            # Initialize default RF here
            default_rf = RandomForestClassifier(random_state=42, n_jobs=-1)
            rf_model, rf_results = self._train_final_model(
                default_rf, X.copy(), y.copy(), 'Random Forest'
            )
            # best_rf = self.model_trainer.tune_random_forest(X, y) # <<< SKIPPED

            best_xgb = self.model_trainer.tune_xgboost(X, y)
            best_gb = self.model_trainer.tune_gradient_boosting(X, y) # Tune GB

            # Check if tuning returned models for XGB and GB
            if not best_xgb or not best_gb:
                 logger.error("Hyperparameter tuning failed for XGBoost or Gradient Boosting. Exiting.")
                 return False

            # Train final models using best hyperparameters found (or defaults for RF)
            # Note: _train_final_model now handles scaling and CALIBRATION internally
            # rf_model, rf_results = self._train_final_model(
            #     best_rf, X.copy(), y.copy(), 'Random Forest') # <<< Use default RF results now
            ...
            # --- Use CALIBRATED models for ensemble --- #
            # Extract calibrated models from results dictionaries
            rf_calibrated = rf_results.get('calibrated_model')
            xgb_calibrated = xgb_results.get('calibrated_model')
            gb_calibrated = gb_results.get('calibrated_model')

            # Determine best weights based on cross-validation performance (from original tuning/training)
            cv_scores = {
                 # Use .get() to safely access scores, provide default if key missing
                'rf': np.mean(rf_results.get('cross_val_scores', [np.nan])), # Score from default RF training
                'xgb': np.mean(xgb_results.get('cross_val_scores', [np.nan])), # Score from tuned XGB training
                'gb': np.mean(gb_results.get('cross_val_scores', [np.nan])) # Score from tuned GB training
            }
            ...
            # Ensure base CALIBRATED models exist before adding to ensemble
            estimators_for_ensemble = []
            if rf_calibrated: estimators_for_ensemble.append(('rf', rf_calibrated))
            if xgb_calibrated: estimators_for_ensemble.append(('xgb', xgb_calibrated))
            if gb_calibrated: estimators_for_ensemble.append(('gb', gb_calibrated))
            ...
            if len(estimators_for_ensemble) < 3:
                 ...
                 if total_available_score > 0:
                      # Calculate weights based on CV scores of the ORIGINAL models before calibration
            ...
            optimized_ensemble = VotingClassifier(
                estimators=estimators_for_ensemble, # Use calibrated estimators
                voting='soft',
            ...
            # NOTE: We retrain the VotingClassifier itself, it uses the predict_proba of its estimators
            opt_ensemble_model, opt_ensemble_results = self._train_final_model(
                optimized_ensemble, X.copy(), y.copy(), 'Optimized Ensemble (Calibrated)' # Updated name
            )
            ...
            self._create_advanced_visualizations(
                ...
                xgb_results,
                gb_results,
                opt_ensemble_results, # Results from ensemble of calibrated models
            ...
            best_model_name = summary.get('best_model_by_accuracy') # Name will include (Calibrated)
            ...
            if best_model_name and best_model_name in model_results_summary:
                 ...
                 best_model_name = 'Optimized Ensemble (Calibrated)' # <<< Adjusted default name
                 results_to_use = opt_ensemble_results
                 model_to_save = results_to_use.get('calibrated_model') # <<< SAVE CALIBRATED ENSEMBLE
                 scaler_to_save = results_to_use.get('scaler')
                 metadata_to_save = summary['metrics'].get(best_model_name, {})
                 if results_to_use.get('feature_names') and len(results_to_use['feature_names']) == len(self.feature_names):
                      feature_names_to_save = results_to_use['feature_names']
