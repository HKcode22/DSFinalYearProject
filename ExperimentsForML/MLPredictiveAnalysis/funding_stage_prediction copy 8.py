...
class EnhancedPipeline(FundingStagePredictionPipeline):
    ...
    def run(self):
        try:
            ...
            # --- Tune Random Forest --- #
            logger.info("Tuning Random Forest (n_iter=8, cv=3)...")
            best_rf = self.model_trainer.tune_random_forest(X, y)
            if not best_rf or isinstance(best_rf, dict): # Check if tuning failed or returned error dict
                 logger.error("Hyperparameter tuning failed for Random Forest. Exiting.")
                 return False
            # Train final tuned RF model
            rf_model, rf_results = self._train_final_model(
                 best_rf, X.copy(), y.copy(), 'Random Forest'
            )

            # --- Tune XGBoost --- #
            logger.info("Tuning XGBoost (n_iter=8, cv=3)...")
            ...
            logger.info("Tuning Gradient Boosting (n_iter=8, cv=3)...") # <<< Set n_iter=8
            best_gb = self.model_trainer.tune_gradient_boosting(X, y, n_iter=8) # <<< Set n_iter=8
            ...
            try:
                # Ensure all base models were successfully trained and calibrated
                if not (rf_results and rf_results.get('calibrated_model') and
                        xgb_results and xgb_results.get('calibrated_model') and
                        gb_results and gb_results.get('calibrated_model')):
                    raise ValueError("One or more base models failed training or calibration.")
                ...
                # Use a consistent scaler (e.g., from GB results)
                scaler_ens = gb_results['scaler']
                X_train_ens_scaled = scaler_ens.transform(X_train_ens)
                X_test_ens_scaled = scaler_ens.transform(X_test_ens)

                # Get calibrated probabilities from base models on TRAINING data
                rf_proba_train = rf_results['calibrated_model'].predict_proba(X_train_ens_scaled)
                xgb_proba_train = xgb_results['calibrated_model'].predict_proba(X_train_ens_scaled)
                gb_proba_train = gb_results['calibrated_model'].predict_proba(X_train_ens_scaled)

                # Create meta-features for training the meta-model
                X_meta_train = np.concatenate((rf_proba_train, xgb_proba_train, gb_proba_train), axis=1)
                ...
                # --- Train a Gradient Boosting meta-model instead --- #
                logger.info(f"Training Gradient Boosting meta-model on {X_meta_train.shape[1]} probability features...")
                meta_model = GradientBoostingClassifier(n_estimators=50, max_depth=3, random_state=42) # Simple GB
                meta_model.fit(X_meta_train, y_train_ens)
                logger.info("GB Meta-model trained.")
                # --- End GB Meta-Model --- #

                # Get calibrated probabilities from base models on TEST data
                rf_proba_test = rf_results['calibrated_model'].predict_proba(X_test_ens_scaled)
                xgb_proba_test = xgb_results['calibrated_model'].predict_proba(X_test_ens_scaled)
                gb_proba_test = gb_results['calibrated_model'].predict_proba(X_test_ens_scaled)

                # Create meta-features for testing
                X_meta_test = np.concatenate((rf_proba_test, xgb_proba_test, gb_proba_test), axis=1)
                ...
                stacking_ensemble_results = {
                    'model': meta_model, # Store the fitted meta-model
                    'calibrated_model': meta_model, # Treat meta-model as the final predictor
                    ...
                    'cross_val_scores': np.array([np.nan]), # CV not performed directly on meta-model here
                    'X_test': X_meta_test, # Store meta-features used for testing
                    'y_test': y_test_ens,
                    'y_pred': y_pred_stack,
                    'y_proba': y_proba_stack,
                    'feature_names': [f'rf_p{i}' for i in range(rf_proba_train.shape[1])] +
                                     [f'xgb_p{i}' for i in range(xgb_proba_train.shape[1])] +
                                     [f'gb_p{i}' for i in range(gb_proba_train.shape[1])], # Meta-feature names
                    'scaler': None # Scaler not directly applicable to meta-model
                }
