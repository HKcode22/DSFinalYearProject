    ...
    def run(self):
        try:
            ...
            model_results_summary = {
                ...
                'Stacking Ensemble (GB Meta)': stacking_ensemble_results # <<< Use stacking results with new name
            }

            summary = self._save_summary(merged_data, X, model_results_summary)

            # Determine the best model to save based on summary results
            best_model_name = summary.get('best_model_by_accuracy') # Name will include (Calibrated) or (GB Meta)
            model_to_save = None # This should be the CALIBRATED model instance
            scaler_to_save = None
            metadata_to_save = {}
            ...
            if best_model_name and best_model_name in model_results_summary:
                 ...
                 best_model_name = 'Stacking Ensemble (GB Meta)' # <<< Adjusted default name
    ...
    def _save_summary(self, merged_data, X, model_results_summary): # model_results_summary includes LGBM and updated ensemble name
        ...
        if best_overall_model_name and model_to_save and scaler_to_save:
            ...
            saved_path = self.model_manager.save_model(
                ...
                model=model_to_save,
                scaler=scaler_to_save,
                feature_names=self.feature_names, # Save with ORIGINAL feature names
                metadata=metadata_to_save
            )
            if saved_path:
            ...
            logger.error(f"Could not save best model '{best_overall_model_name}' - essential components missing.")
        # --- End Saving Call --- #
