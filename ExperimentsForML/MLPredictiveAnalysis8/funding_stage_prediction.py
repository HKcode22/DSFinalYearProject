...
class AdvancedVisualizer(Visualizer):
    ...
    def plot_calibration(self, y_true, y_proba, n_bins=15): # <<< Changed n_bins to 15
        plt.figure(figsize=(10, 6))
        if y_proba is None:
        ...
        for class_idx in range(n_classes):
            ...
            try:
                ...
                prob_true, prob_pred = calibration_curve(
                    binary_y, class_proba, n_bins=n_bins, strategy='uniform' # <<< Changed strategy to 'uniform'
                )
                # +++ Add check for empty prob_true/prob_pred +++
                if len(prob_true) > 0 and len(prob_pred) > 0:
                    plt.plot(prob_pred, prob_true, marker='o', linestyle='-',
                             label=f'Class {class_idx}', alpha=0.7)
                else:
                    logger.info(f"Skipping plot for class {class_idx} as calibration_curve returned empty arrays.")
                # --- End check ---
...
class EnhancedPipeline(FundingStagePredictionPipeline):
    ...
    def _save_summary(self, merged_data, X, model_results_summary, processed_data):
        ...
        # +++ Log entry into stacking save block +++
        logger.info(f"Checking if Stacking Ensemble results exist for dashboard saving (Key: '{stacking_key}')...")
        if stacking_key in model_results_summary and model_results_summary[stacking_key]:
            # +++ Log that the block was entered +++
            logger.info(f"Found Stacking Ensemble results. Proceeding with dashboard save checks.")
            stacking_results_data = model_results_summary[stacking_key]
            stacking_model_to_save = stacking_results_data.get('model') # The ensemble model itself
            stacking_scaler_to_save = stacking_results_data.get('scaler')
            stacking_feature_names = stacking_results_data.get('feature_names') # Should be original features
            stacking_metrics = summary['metrics'].get(stacking_key, {})
            # Anomaly detector for ensemble is complex, usually None or handled differently
            stacking_anomaly_detector = stacking_results_data.get('anomaly_detector')

            # +++ Log components found +++
            logger.info(f"  - Stacking Model found: {stacking_model_to_save is not None}")
            logger.info(f"  - Stacking Scaler found: {stacking_scaler_to_save is not None}")
            logger.info(f"  - Stacking Feature Names found: {stacking_feature_names is not None}")

            if stacking_model_to_save and stacking_scaler_to_save and stacking_feature_names:
                dashboard_stacking_name = f"Dashboard_Model_{stacking_save_name_suffix}"
                # +++ Log final save attempt +++
                logger.info(f"Attempting final save for dashboard stacking model: {dashboard_stacking_name}")
                stacking_dashboard_saved_path = self.model_manager.save_model(
                    model_name=dashboard_stacking_name,
                    model=stacking_model_to_save,
                    scaler=stacking_scaler_to_save,
                    feature_names=stacking_feature_names,
                    metadata={'training_metadata': stacking_metrics,
                              'class_mapping': getattr(self, 'final_index_to_string_label_map', {})},
                    anomaly_detector=stacking_anomaly_detector # Pass if available, likely None
                )
                if stacking_dashboard_saved_path:
                    logger.info(f"Successfully saved {stacking_key} with dashboard prefix to {stacking_dashboard_saved_path}")
                    if not os.path.exists(stacking_dashboard_saved_path):
                        logger.error(f"!!! FILE NOT FOUND AFTER SAVING STACKING DASHBOARD MODEL: {stacking_dashboard_saved_path} !!!")
                    else:
                        logger.info(f"Confirmed stacking dashboard model file exists: {stacking_dashboard_saved_path}")
                else:
                    logger.error(f"Failed to save {stacking_key} with dashboard prefix.")
            else:
                logger.warning(f"Skipping dashboard save for {stacking_key} - missing model, scaler, or feature names in results.")
        else:
            logger.warning(f"Stacking Ensemble results ({stacking_key}) not found or invalid. Cannot save explicitly for dashboard.")
        # --- End Explicit Stacking Ensemble Save for Dashboard --- #
