# Trivial change to force update: 2025-05-12
try:
    ...
    def _parse_funding_amount(self, amount_str):
        ...
        try:
            ...
            if value > max_reasonable_amount:
                logger.warning(f"Unreasonably large funding amount detected: ${value:,.2f}")
                return np.nan
            elif value < 0:
    ...
    def merge_datasets(self):
        ...
        try:
            ...
            if not growthlist_df.empty:
                ...
                logger.info(
                    f"Processed {len(growthlist_df)} records from growthlist data, rejected {len(rejected_records) - growthlist_rejected} suspicious records")
            ...
            if not topstartup_df.empty:
                ...
                logger.info(
                    f"Processed {len(topstartup_df)} records from topstartup data, rejected {len(rejected_records) - topstartup_rejected} suspicious records")
            ...
            if not rejected_df.empty:
                ...
                rejected_df.to_csv(
                    ...
                    index=False)
                logger.info(
                    f"Saved {len(rejected_df)} rejected records to {rejected_path}")
            ...
            if all_records:
                ...
                logger.info(
                    f"Merged data columns: {merged_data.columns.tolist()}")
                logger.info(
                ...
                logger.info(
                    f"Removed {pre_dedup_count - len(merged_data)} duplicate records")
                ...
                logger.info(
                    f"Successfully merged {len(merged_data)} validated records")
                return merged_data
...
class FeatureEngineering:
    ...
    def extract_features(self, df):
        ...
        logger.info(
            f"Found {len(valid_stages)} unique funding stages: {valid_stages}")
        ...
        try:
            ...
            if data['funding_date'].isna().sum() > len(data) * 0.3:
                logger.warning(
                    f"Many dates failed to parse ({data['funding_date'].isna().sum()} NaN values)")
                ...
                for fmt in ['%d-%b-%y', '%b %Y', '%Y', '%m/%d/%Y', '%Y-%m-%d']:
                    try:
                        ...
                        if na_before > na_after:
                            logger.info(
                                f"Format '{fmt}' parsed {na_before - na_after} dates")
                    except BaseException:
                        pass
        ...
        logger.info(
            f"Feature engineering complete: {data.shape[1]} features created")
        return data
    ...
    def prepare_model_data(self, data):
        ...
        if y.notna().sum() < 10:
            logger.warning(
                f"Very few valid target values: {y.notna().sum()} out of {len(y)}")
        ...
        logger.info(
            f"Prepared model data: X shape={X.shape}, y shape={y.shape}")
    ...
    def train_random_forest(self, X, y):
        ...
        try:
            ...
            if outliers_mask.any():
                logger.info(
                    f"Identified {outliers_mask.sum()} potential outliers in training data")
                ...
                if outlier_examples is not None:
                    logger.info(
                        f"Outlier examples:\n{outlier_examples.to_dict(orient='records')}")
            ...
            model_path = os.path.join(
                self.output_dir, f"random_forest_{self.timestamp}.joblib")
            model_metadata = {
                'model': rf,
...
class Visualizer:
    ...
    def plot_funding_stage_distribution(self, data, stage_mapping_rev=None): # Add stage_mapping_rev parameter
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"funding_stage_dist_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_confusion_matrices(self, model_results):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"confusion_matrices_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_funding_vs_employees(self, data):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"funding_vs_employees_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_correlation_heatmap(self, data):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"correlation_heatmap_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_temporal_trends(self, data):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"temporal_trends_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_industry_distributions(self, data):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"industry_analysis_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_pairwise_features(self, data, features):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"pairwise_features_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_full_correlation_heatmap(self, data):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"full_correlation_heatmap_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_violin_funding_by_stage(self, data):
        ...
        plt.savefig(
            os.path.join(
                self.output_dir,
                f"violin_funding_by_stage_{self.timestamp}.png"))
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def detect_anomalies(self, X, company_name=None, threshold=-0.5):
        ...
        try:
            ...
            if X.shape[1] != len(self.feature_ranges['min']):
                # Log the dimension mismatch error
                expected_dims = len(self.feature_ranges['min'])
                actual_dims = X.shape[1]
                error_message = f"Feature dimension mismatch: expected {expected_dims}, got {actual_dims}"
                logger.error(error_message)
                # Return anomaly due to dimension mismatch
                return {
                    'is_anomaly': True,
            ...
            # Log the general anomaly detection error (Simplified)
            logger.error("Error detecting anomalies") 
            logger.error(e, exc_info=True) # Log the actual exception details separately
            # Return anomaly due to detection error
            return {
                'is_anomaly': True,
                'reason': 'detection_error', # Simplified reason
                'score': 1.0
            }
