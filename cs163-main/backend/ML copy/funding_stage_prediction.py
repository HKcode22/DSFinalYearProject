...
class EnhancedModelTrainer(ModelTrainer):
    def tune_random_forest(self, X, y):
        ...
        try:
            # Simplified param_grid for testing responsiveness
            param_grid = {
                'n_estimators': randint(100, 500), # Reduced range
                'max_depth': [None, 10, 20, 30], # Fewer discrete values
                'min_samples_split': randint(2, 20), # Reduced upper bound
                'min_samples_leaf': randint(1, 20),  # Reduced upper bound
                'max_features': ['sqrt', 'log2', None],
                'bootstrap': [True, False],
                'class_weight': ['balanced', None], # Simplified
                'criterion': ['gini', 'entropy'], # Simplified
                'max_leaf_nodes': [None, 50, 100, 150], # Simplified
                'min_impurity_decrease': [0.0, 0.001, 0.01], # Simplified
                'ccp_alpha': uniform(0.0, 0.05), # Simplified range
                'min_weight_fraction_leaf': [0.0, 0.1, 0.2] # Simplified options
            }
            logger.info(f" EXPERIMENTAL: Using deliberately simple param_grid for Random Forest: {param_grid}")

            rf = RandomForestClassifier(random_state=42)
            grid_search = RandomizedSearchCV(
                estimator=rf,
                param_distributions=param_grid,
                n_iter=1,  # Drastically reduced n_iter
                cv=2,  # Drastically reduced cv
                verbose=1,
                random_state=42,
                n_jobs=-1,
                scoring='accuracy',
    ...
    def tune_xgboost(self, X, y):
        ...
        # Define parameter grid (Simplified Version for faster testing)
        param_grid = {
            'n_estimators': randint(100, 1000), # Reduced range
            'max_depth': randint(3, 15), # Reduced range
            'learning_rate': [0.01, 0.05, 0.1, 0.2], # Simplified list
            'subsample': uniform(0.6, 0.4), # Samples from 0.6 to 1.0
            'colsample_bytree': uniform(0.6, 0.4), # Samples from 0.6 to 1.0
            'min_child_weight': randint(1, 10), # Reduced range
            'gamma': uniform(0, 1.0), # Reduced range
            'reg_alpha': [0, 0.01, 0.1, 1], # Simplified list
            'reg_lambda': [0.01, 0.1, 1, 5],    # Simplified list
            'max_delta_step': [0, 1, 5], # Simplified
            'objective': ['multi:softprob'], 
            'booster': ['gbtree', 'dart'], # Simplified
            'tree_method': ['auto', 'hist'], # Simplified
            'scale_pos_weight': [1], # Simplified
            'grow_policy': ['depthwise', 'lossguide'] # Simplified
        }
    ...
    def tune_gradient_boosting(self, X, y, n_iter): # <<< Changed default n_iter back to 4
        ...
        param_grid = {
            ...
            'min_impurity_decrease': [0.0, 0.001, 0.01], # Simplified
            'max_leaf_nodes': [None, 50, 100], # Simplified
            'ccp_alpha': uniform(0.0, 0.05), # Simplified range
            'init': [None], # Simplified
            'validation_fraction': [0.1], # Simplified
        ...
        search = RandomizedSearchCV(
            estimator=gb,
            param_distributions=param_grid,
            n_iter=n_iter,  # Use passed n_iter value (pipeline sets this to 5)
            cv=3,     # Set cv=3 for speed
            verbose=1,
            random_state=42,
    ...
    def train_catboost(self, X, y):
        ...
        try:
            ...
            param_grid = {
                'iterations': randint(100, 300), # Reduced range
                'learning_rate': uniform(0.01, 0.2), # Reduced range
                'depth': randint(4, 8), # Reduced range
                'l2_leaf_reg': uniform(1, 5), # Reduced range
                'bagging_temperature': [0.5, 1.0], # Simplified
                'random_strength': [0.5, 1.0], # Simplified
                'grow_policy': ['SymmetricTree', 'Depthwise'] # Simplified
            }
            ...
            search = RandomizedSearchCV(
                cat_clf, param_grid, n_iter=3, cv=3, # <<< Reduced n_iter and cv
    ...
    def tune_lightgbm(self, X, y, n_iter=3): # <<< SET n_iter default to 30
        """Tune LightGBM classifier using RandomizedSearchCV"""
        if LGBMClassifier is None:
            logger.warning("LightGBM not installed. Skipping tuning.")
            return None
        ...
        param_grid = {
            ...
            'learning_rate': uniform(0.01, 0.15), # Slightly reduced range
            'num_leaves': randint(20, 40), # Reduced range
            'max_depth': [-1, 5, 10], # Simplified
            'min_child_samples': randint(10, 50), # Adjusted range
            'subsample': uniform(0.6, 0.4), # range 0.6 to 1.0
            'colsample_bytree': uniform(0.6, 0.4), # range 0.6 to 1.0
            'reg_alpha': [0, 0.01, 0.1], # Simplified
            'reg_lambda': [0, 0.01, 0.1], # Simplified
            'boosting_type': ['gbdt', 'dart'], # Simplified
            'max_bin': randint(100, 300) # Reduced range
        }
        ...
        search = RandomizedSearchCV(
            estimator=lgbm,
            param_distributions=param_grid,
            n_iter=n_iter, # Use the passed n_iter (pipeline sets this to 5)
            cv=3, # Set cv=3
            verbose=1,
            random_state=42,
            scoring='accuracy',
    ...
    def tune_decision_tree(self, X, y, n_iter=3): # <<< SET n_iter default to 30
        """Tune Decision Tree using RandomizedSearchCV."""
        ...
        param_grid = {
            ...
            'max_depth': [None, 10, 20, 30], # Simplified range
            'min_samples_split': randint(2, 30), # Reduced range
            'min_samples_leaf': randint(1, 30), # Reduced range
            'max_features': ['sqrt', 'log2', None, 0.5, 0.7], # Simplified options
            'min_impurity_decrease': [0.0, 0.001, 0.01], # Simplified
            'ccp_alpha': uniform(0.0, 0.05), # Simplified range
        ...
        search = RandomizedSearchCV(
            estimator=dt,
            param_distributions=param_grid,
            n_iter=n_iter, # Use passed n_iter (pipeline sets this to 5)
            cv=3, # Set cv=3
            verbose=1,
            random_state=42,
            scoring='accuracy',
...
class Visualizer:
    ...
    def plot_rmse_comparison(self, model_results):
        ...
        for model_name, results in model_results.items():
            if results and 'rmse' in results and pd.notna(results['rmse']):
                models.append(model_name)
                rmses.append(results['rmse'])
            else:
                logger.warning(f"RMSE not found or is NaN for model: {model_name}. Skipping in RMSE plot.")

        if not models: # Check if any models had valid RMSE
            logger.warning("No models with valid RMSE found to plot.")
            plt.close() # Close the empty figure
            return

        # Sort by RMSE for better visualization (lower is better)
        sorted_indices = np.argsort(rmses)
        models = [models[i] for i in sorted_indices]
        rmses = [rmses[i] for i in sorted_indices]

        # Use a consistent color, perhaps highlight the best (lowest RMSE)
        colors = ['lightcoral'] * len(models)
        if rmses: # Check if rmses list is not empty
            best_model_index = 0 # Since it's sorted, the first one is the best
            colors[best_model_index] = 'firebrick'
        ...
        for bar in bars:
            height = bar.get_height()
            plt.text(bar.get_x() + bar.get_width() / 2., height + 0.005, # Adjust offset slightly if needed
                     f'{height:.4f}', ha='center', va='bottom')

        plt.tight_layout()
        plot_path = os.path.join(self.output_dir, f"rmse_model_comparison_{self.timestamp}.png")
        plt.savefig(plot_path)
        logger.info(f"RMSE comparison plot saved to {plot_path}")
        if self.interactive:
            plt.show()
        plt.close()
    ...
    def plot_calibration(self, y_true, y_proba, model_name, n_bins=5):
        ...
        for class_idx in range(n_classes):
            ...
            if class_idx not in unique_true_classes:
                 logger.info(f"Skipping calibration plot for class {class_idx} of {model_name} as it's not present in y_true.")
                 continue
            ...
            try:
                ...
                if np.sum(binary_y) == 0:
                     logger.info(f"Skipping calibration plot for class {class_idx} of {model_name} - no positive samples in y_true for this class.")
                     continue
                ...
                logger.warning(
                    f"Skipping calibration for class {class_idx} of {model_name}: { # <<< Error was here
...
class EnhancedPipeline(FundingStagePredictionPipeline):
    def __init__(self, *args, **kwargs):
        # Determine project_main_dir (cs163-main) from this script's location
        # cs163-main/backend/ML/funding_stage_prediction.py
        script_path = os.path.abspath(__file__)
        ml_dir = os.path.dirname(script_path)  # cs163-main/backend/ML
        backend_dir = os.path.dirname(ml_dir) # cs163-main/backend
        project_main_dir = os.path.dirname(backend_dir) # cs163-main

        # Override the output_dir with our custom path if not already correctly set by args
        # The goal is for self.output_dir to be cs163-main/backend/MainOutput
        # args[1] or kwargs['output_dir'] from main() should ideally pass this.
        # If called directly, calculate it.
        
        effective_output_dir = None
        if 'output_dir' in kwargs:
            effective_output_dir = kwargs['output_dir']
        elif len(args) > 1 and args[1] is not None: # args[1] is output_dir
            effective_output_dir = args[1]
        
        if not effective_output_dir: # If not passed by main(), calculate default
            effective_output_dir = os.path.join(backend_dir, 'MainOutput')
            logger.info(f"EnhancedPipeline calculated default output_dir: {effective_output_dir}")

        # kwargs['output_dir'] will be used by super().__init__
        kwargs['output_dir'] = effective_output_dir
        
        # Adjust base_dir if not explicitly passed, to point to AdataCollection/JSONFolder
        # args[0] is base_dir
        if 'base_dir' not in kwargs and (len(args) == 0 or args[0] is None):
            # Default base_dir for DataLoader should be relative to cs163-main/backend
            # e.g., AdataCollection/JSONFolder if JSONFolder is inside AdataCollection
            kwargs['base_dir'] = os.path.join(backend_dir, 'AdataCollection', 'JSONFolder') # Example
            logger.info(f"EnhancedPipeline calculated default base_dir for DataLoader: {kwargs['base_dir']}")
        elif 'base_dir' in kwargs and kwargs['base_dir'] == './JSONFolder': # If main passed the old default
             kwargs['base_dir'] = os.path.join(backend_dir, 'AdataCollection', 'JSONFolder')
             logger.info(f"EnhancedPipeline adjusted relative base_dir to: {kwargs['base_dir']}")


        super().__init__(*args, **kwargs) # This will set self.output_dir
        
        # self.models_dir, self.viz_dir are set by super based on self.output_dir
        # self.model_trainer path is also based on self.models_dir

        # Ensure components use the corrected self.output_dir
        self.data_loader = DataLoader(
            base_dir=self.base_dir, # This is super().base_dir, which got from args/kwargs
            archive=kwargs.get('archive', False), 
            output_dir_for_db=self.output_dir # Pass the main output dir for the DB
        )
        self.model_trainer = EnhancedModelTrainer(self.models_dir) # Uses self.models_dir from super
        self.visualizer = AdvancedVisualizer(self.viz_dir, interactive=False) # Uses self.viz_dir from super
        self.model_manager = ModelManager(self.models_dir)
        self.time_series_forecaster = TimeSeriesForecaster(self.viz_dir) 
        self._init_model_directory() # Uses self.models_dir
        ...
        self.tuning_iterations = 5 # <<< REDUCED to 5 for faster test run (for GB, LightGBM, DT)
    ...
    def run(self):
        try:
            ...
            # --- Train and Evaluate Main Models ---
            logger.info("*** Starting Main Model Training and Tuning Loop ***")

            # Split data for main model training and tuning
            # It's generally better to tune on a subset and then train final model on full training set
            # However, for consistency with how _train_final_model splits, we'll pass full X, y to tuning
            # and let _train_final_model handle its own split.
            # The tuning functions themselves might do an internal split or use CV.

            # --- Random Forest ---
            logger.info("--- Tuning Random Forest ---")
            tuned_rf_model = self.model_trainer.tune_random_forest(X.copy(), y.copy())
            if tuned_rf_model and not isinstance(tuned_rf_model, dict): # Check if tuning succeeded
                logger.info("Training final Random Forest model...")
                _, rf_results = self._train_final_model(
                    tuned_rf_model, X.copy(), y.copy(), 'Random Forest (Calibrated)'
                )
                if rf_results: all_model_results['Random Forest (Calibrated)'] = rf_results
            else:
                logger.error("Random Forest tuning failed or returned error. Skipping final model.")
                all_model_results['Random Forest (Calibrated)'] = None


            # --- XGBoost ---
            logger.info("--- Tuning XGBoost ---")
            tuned_xgb_model = self.model_trainer.tune_xgboost(X.copy(), y.copy())
            if tuned_xgb_model and not isinstance(tuned_xgb_model, dict): # Check if tuning succeeded
                logger.info("Training final XGBoost model...")
                _, xgb_results = self._train_final_model(
                    tuned_xgb_model, X.copy(), y.copy(), 'XGBoost (Calibrated)'
                )
                if xgb_results: all_model_results['XGBoost (Calibrated)'] = xgb_results
            else:
                logger.error("XGBoost tuning failed or returned error. Skipping final model.")
                all_model_results['XGBoost (Calibrated)'] = None

            # --- Gradient Boosting ---
            logger.info("--- Tuning Gradient Boosting ---")
            # Note: tune_gradient_boosting takes n_iter as an argument
            tuned_gb_model = self.model_trainer.tune_gradient_boosting(
                X.copy(), y.copy(), n_iter=self.tuning_iterations # Use pipeline's n_iter
            )
            if tuned_gb_model and not isinstance(tuned_gb_model, dict): # Check if tuning succeeded
                logger.info("Training final Gradient Boosting model...")
                _, gb_results = self._train_final_model(
                    tuned_gb_model, X.copy(), y.copy(), 'Gradient Boosting (Calibrated)'
                )
                if gb_results: all_model_results['Gradient Boosting (Calibrated)'] = gb_results
            else:
                logger.error("Gradient Boosting tuning failed or returned error. Skipping final model.")
                all_model_results['Gradient Boosting (Calibrated)'] = None

            # --- LightGBM ---
            if LGBMClassifier is not None:
                logger.info("--- Tuning LightGBM ---")
                tuned_lgbm_model = self.model_trainer.tune_lightgbm(
                    X.copy(), y.copy(), n_iter=self.tuning_iterations # Use pipeline's n_iter
                )
                if tuned_lgbm_model and not isinstance(tuned_lgbm_model, dict): # Check if tuning succeeded
                    logger.info("Training final LightGBM model...")
                    _, lgbm_results = self._train_final_model(
                        tuned_lgbm_model, X.copy(), y.copy(), 'LightGBM (Calibrated)'
                    )
                    if lgbm_results: all_model_results['LightGBM (Calibrated)'] = lgbm_results
                else:
                    logger.error("LightGBM tuning failed or returned error. Skipping final model.")
                    all_model_results['LightGBM (Calibrated)'] = None
            else:
            ...
            tuned_dt_model = self.model_trainer.tune_decision_tree(
                X.copy(), y.copy(), n_iter=self.tuning_iterations # Use pipeline's n_iter
            )
            if tuned_dt_model and not isinstance(tuned_dt_model, dict): # Check if tuning succeeded
                logger.info("Training final Decision Tree model...")
                _, dt_results = self._train_final_model(
                    tuned_dt_model, X.copy(), y.copy(), 'Decision Tree (Calibrated)'
                )
                if dt_results: all_model_results['Decision Tree (Calibrated)'] = dt_results
            else:
                logger.error("Decision Tree tuning failed or returned error. Skipping final model.")
                all_model_results['Decision Tree (Calibrated)'] = None
            
            # --- Initialize other model results to None if they weren't trained/tuned ---
            # (This ensures keys exist for visualization and summary, even if models failed)
            if 'SVC (Linear, Calibrated)' not in all_model_results: all_model_results['SVC (Linear, Calibrated)'] = None
            if 'Logistic Regression (Calibrated)' not in all_model_results: all_model_results['Logistic Regression (Calibrated)'] = None
            if 'K-Nearest Neighbors (Calibrated)' not in all_model_results: all_model_results['K-Nearest Neighbors (Calibrated)'] = None
            if 'MLP Classifier (Calibrated)' not in all_model_results: all_model_results['MLP Classifier (Calibrated)'] = None
            if 'Stacking Ensemble (LR Meta)' not in all_model_results: all_model_results['Stacking Ensemble (LR Meta)'] = None


            # --- Prepare Data & Train/Save Dashboard-Specific Model (KEEP THIS PART) --- #
            ...
            lgbm_results = None
            if LGBMClassifier is not None:
                logger.info("Training LightGBM with defaults...")
                try:
                    default_lgbm = LGBMClassifier(random_state=42, n_jobs=-1, class_weight='balanced') # Added class_weight
                    lgbm_model, lgbm_results = self._train_final_model(
                        default_lgbm, X.copy(), y.copy(), 'LightGBM'
                    )
                    if lgbm_results: all_model_results['LightGBM (Calibrated)'] = lgbm_results
                except Exception as lgbm_err:
                    logger.error(f"Error training default LightGBM: {lgbm_err}", exc_info=True)
                    lgbm_model, lgbm_results = None, None # Ensure they are None on error
            else:
            ...
            self._create_advanced_visualizations(
                ...
                all_model_results.get('LightGBM (Calibrated)'), # Add LightGBM results
    ...
    def _train_final_model(self, model, X, y, name):
        ...
        try:
            ...
            cv_scores = cross_val_score(model_clone, X_train_scaled, y_train, cv=5, scoring='accuracy', error_score='raise') # Enabled with error_score
    ...
    def _create_advanced_visualizations(
            ...
            lgbm_results, # Add LightGBM results
        ...
        try:
            ...
            if lgbm_results and lgbm_results.get('model'): all_results_plot['LightGBM'] = lgbm_results
            ...
            for model_name, results in all_results_plot.items():
                 ...
                 if results and results.get('y_proba') is not None and results.get('y_test') is not None and results.get('y_pred') is not None:
                     ...
                     if len(current_classes) > 1: # Need at least 2 classes for ROC/Calibration
                         ...
                         try:
                             # Determine n_bins based on model_name
                             if any(term in model_name for term in ["LightGBM", "Gradient Boosting", "XGBoost", "Random Forest"]):
                                 n_bins_for_model = 10
                             elif any(term in model_name for term in ["Decision Tree", "Stacking Ensemble"]):
                                 n_bins_for_model = 6
                             else:
                                 n_bins_for_model = 10 # Default for others
                             
                             logger.info(f"Using n_bins={n_bins_for_model} for calibration plot of {model_name}")
                             self.visualizer.plot_calibration(
                                 y_test_current,
                                 y_proba_current,
                                 model_name,
                                 n_bins=n_bins_for_model) # Pass model_name and n_bins
            ...
            base_models_for_fi = {
                ...
                'LightGBM': lgbm_results.get('model') if lgbm_results else None
            }
            base_results_for_fi = {
                'Random Forest': rf_results,
                'XGBoost': xgb_results,
            ...
            for model_name, model_instance in base_models_for_fi.items():
                 results_dict = base_results_for_fi.get(model_name, {})
                 # --- ADDED checks for model_instance and results_dict --- #
                 if not model_instance:
                     logger.info(f"Skipping FI for {model_name}: model_instance is None.")
                     continue
                 if not results_dict:
                     logger.info(f"Skipping FI for {model_name}: results_dict is empty or None.")
                     continue
                 ...
                 # Now all conditions are met, proceed with plotting
                 try:
            ...
            if all_results_plot:
                ...
                self.visualizer.plot_rmse_comparison(all_results_plot) # Call new RMSE comparison plot
            ...
            if processed_data is not None and not processed_data.empty and hasattr(self, 'final_index_to_string_label_map'):
                try:
                    ...
                    self.visualizer.plot_funding_vs_employees(processed_data) # Call funding_vs_employees plot
...
def main():
    ...
    try:
        script_path = os.path.abspath(__file__)
        ml_dir = os.path.dirname(script_path)  # cs163-main/backend/ML
        backend_dir_abs = os.path.dirname(ml_dir) # cs163-main/backend
        project_main_dir_abs = os.path.dirname(backend_dir_abs) # cs163-main
    except NameError: # __file__ might not be defined
        project_main_dir_abs = os.path.abspath(os.path.join(os.getcwd(), '..', '..')) # Guess if running interactively
        logger.warning(f"__file__ not defined, guessing project_main_dir_abs: {project_main_dir_abs}")

    default_base_dir = os.path.join(backend_dir_abs, 'AdataCollection', 'JSONFolder')
    default_output_dir_main = os.path.join(backend_dir_abs, 'MainOutput')

    parser.add_argument(
        '--base_dir',
        type=str,
        default=default_base_dir,
        help='Base directory for data files (e.g., where JSONFolder is)')
    
    # Calculate the default output directory relative to cs163-main/backend
    # This was already corrected, but let's ensure it aligns with the new structure if run standalone
    # script_dir = os.path.dirname(os.path.abspath(__file__))
    # project_backend_dir = os.path.dirname(script_dir) # This is cs163-main/backend/ML
    # project_main_dir_for_output = os.path.dirname(os.path.dirname(project_backend_dir)) # This should be cs163-main
    # default_output_dir = os.path.join(project_main_dir_for_output, 'backend', 'MainOutput')
    # The line above pointed to cs163-main/backend/MainOutput, which is correct.
    # The default_output_dir_main calculated earlier is also cs163-main/backend/MainOutput.

    parser.add_argument(
        '--output_dir',
        type=str,
        default=default_output_dir_main,
        help='Output directory for results (e.g., cs163-main/backend/MainOutput)')
    parser.add_argument(
        '--archive',
        action='store_true',
    ...
    # Pass absolute paths or paths that EnhancedPipeline can correctly interpret
    pipeline = EnhancedPipeline(
        base_dir=args.base_dir, # This will be absolute or relative to execution
        output_dir=args.output_dir, # This will be absolute or relative to execution
