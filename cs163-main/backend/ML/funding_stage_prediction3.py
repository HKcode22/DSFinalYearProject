...
def main():
    ...
    parser.add_argument(
        '--base_dir',
        type=str,
        default='./JSONFolder', # This will be relative to where the script is run (cs163-main/backend)
        help='Base directory for data files')
    
    # Calculate the default output directory relative to cs163-main
    # __file__ in this context is cs163-main/backend/funding_stage_prediction.py
    # So, two os.path.dirname calls will get to cs163-main
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_main_dir = os.path.dirname(script_dir) # This should be 'cs163-main'
        default_output_dir = os.path.join(project_main_dir, 'MainOutput')
    except NameError: # __file__ might not be defined if not run as script
        default_output_dir = './MainOutput' # Fallback if path calculation fails
        logger.warning(f"Could not auto-determine project root for default output_dir, using relative '{default_output_dir}'")
