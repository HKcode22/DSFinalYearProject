...
class FeatureEngineering:
    ...
    def extract_features(self, df):
        ...
        # Standardize industry categories before using it in history features
        data['industry_category'] = data['industry'].fillna('Unknown') # <<< MOVED HERE
        ...
        # data['industry_category'] = data['industry'].fillna('Unknown') # <<< ORIGINAL POSITION - REMOVED/COMMENTED
...
class EnhancedModelTrainer(ModelTrainer):
    def tune_random_forest(self, X, y):
        ...
        try: # Add try block
            ...
            grid_search = RandomizedSearchCV(
                ...
                n_iter=5, # <<< Increased n_iter from 2 to 5
                cv=3,     # (Keep cv=3)
                verbose=1,
                random_state=42,
                n_jobs=-1,
    ...
    def tune_xgboost(self, X, y):
        ...
        param_grid = {
            ...
            'max_depth': randint(3, 10), # Reduced range
            'learning_rate': [0.01, 0.05, 0.1, 0.2], # Reduced options
            'subsample': uniform(0.6, 0.4), # Samples from 0.6 to 1.0 (range is 0.4)
            'colsample_bytree': uniform(0.6, 0.4), # Samples from 0.6 to 1.0
            # 'colsample_bylevel': uniform(0.5, 0.5), # Removed for speed
            # 'colsample_bynode': uniform(0.5, 0.5),  # Removed for speed
            'min_child_weight': randint(1, 10), # Slightly reduced range
            'gamma': uniform(0, 0.5), # Reduced range
            'reg_alpha': [0, 0.01, 0.1, 1], # Reduced options
            'reg_lambda': [0.01, 0.1, 1, 5],    # Reduced options
            'max_delta_step': [0, 1, 5], # Reduced options
            'objective': ['multi:softprob'],
            'booster': ['gbtree', 'dart'],
            'tree_method': ['hist'], # Using hist for potentially faster training
            'scale_pos_weight': [1] + list(uniform(1, 5).rvs(2)) # Reduced rvs for imbalanced data
        }
        ...
        search = RandomizedSearchCV(
            estimator=model,
            param_distributions=param_grid,
            n_iter=2, # Kept n_iter at 2 for XGBoost for speed
            cv=3,    # (Keep cv=3)
            verbose=2,
            random_state=42,
            n_jobs=-1,
    ...
    def run(self):
        try:
            ...
            if tuned_rf_model_candidate and not isinstance(tuned_rf_model_candidate, dict): # Check if tuning returned a model
                ...
                rf_model_to_train = RandomForestClassifier(random_state=42, n_jobs=-1, class_weight='balanced', n_estimators=50)
            ...
            # --- Gradient Boosting --- #
            logger.info("Tuning Gradient Boosting model (n_iter=2)...")
            ...
            # --- Decision Tree --- #
            logger.info("Tuning Decision Tree model (n_iter=2)...")
            ...
            if LGBMClassifier is not None:
                logger.info("Tuning LightGBM model (n_iter=2)...")
                tuned_lgbm_model_candidate = self.model_trainer.tune_lightgbm(X.copy(), y.copy(), n_iter=2)
                if tuned_lgbm_model_candidate and not isinstance(tuned_lgbm_model_candidate, dict):
                    ...
                    try:
                        lgbm_model_to_train = LGBMClassifier(random_state=42, n_jobs=-1, class_weight='balanced')
